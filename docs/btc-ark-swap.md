...
Build `bark`, then point `BARK` at the rebuilt binary:

```sh
cargo build -p bark-cli --bin bark
...
Create the signet wallet and get an on-chain address to fund:
...
export SIGNET_FUNDING_ADDRESS="$($BARK --quiet --datadir "$DATADIR" onchain address | jq -r '.address')"
echo "$SIGNET_FUNDING_ADDRESS"
```

Send signet coins to `SIGNET_FUNDING_ADDRESS`, then wait for confirmation. In a
one-wallet loopback, this same wallet needs enough on-chain funds for both
sides of the POC:

- board `SWAP_AMOUNT` into a spendable Ark VTXO for the logical `ark_payer`
- fund the BTC lock for the logical `btc_payer`
- pay on-chain transaction fees for both operations

Check the on-chain balance before boarding:

```sh
$BARK --datadir "$DATADIR" onchain balance
```

Fund the Ark side by boarding from the on-chain wallet, then wait until the Ark
VTXO is spendable:
...
The `ark-transfer` step spends Ark VTXOs from the `ark_payer` wallet. Before
running it, `balance` must show at least the swap amount in `spendable_sat`. If
the amount is in `pending_board_sat`, wait for the board transaction to confirm
and run `balance` again. If `spendable_sat` and `pending_board_sat` are both
zero, the wallet you are using for the Ark payer has not boarded funds.

In a real two-wallet test, the BTC payer wallet needs on-chain signet BTC for
`btc-fund`, while the Ark payer wallet needs spendable Ark VTXOs for
`ark-transfer`. The Ark payer can get those VTXOs by boarding from its own
on-chain wallet or by receiving an Ark payment to an address from
`$BARK --datadir "$ARK_PAYER_DATADIR" address`.
...
```json
{ "status": "ArkTransferred", "next": "ark-sign-btc-claim" }
```
...
```json
{ "status": "BtcClaimPartiallySigned", "next": "btc-build-claim-adaptor" }
```
